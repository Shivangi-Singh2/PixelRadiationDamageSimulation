...
void run3HamburgandSheffield() {
    ...
    TGraphErrors *graph2 = (TGraphErrors*)_file0->Get("I_leak_hamburg");
    TGraphErrors *graph3 = (TGraphErrors*)_file0->Get("I_leak_sheffield");
...
// Middle pad with Leakage current Data and simulation
 ...
 // Computing leakage current at T ± 2K using scaling factor
	...
	band->SetFillColorAlpha(kGreen - 10, 0.5);
       ...
       TLegend *legend = new TLegend(0.4, 0.72, 0.8, 0.88);
       ...
       legend->AddEntry(graph2, "Leakage current (Hamburg Simulation)", "lp");
       legend->AddEntry(graph3, "Leakage current (Sheffield Simulation)", "lp");
